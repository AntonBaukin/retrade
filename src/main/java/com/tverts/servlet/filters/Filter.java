...
public interface Filter
{
	...
	/**
	 * Does the main work of the filter.
	 *
	 * If the filter returns without invoking
	 * {@link FilterTask#continueCycle()} the
	 * cycle of the filters invocation
	 * is continued without nesting the filters
	 * calls in the stack. In this case all
	 * deinitialization of the filtering state
	 * must be done in
	 * {@link #closeFilter(FilterTask)}.
	 *
	 * If the filter needs to create a nested
	 * call, it may invoke (also, indirectly)
	 * {@link FilterTask#continueCycle()}
	 * to continue the cycle. Transaction scopes
	 * are created in this way.
	 *
	 * It is possible to break the cycle of
	 * filtering: call {@link FilterTask#setBreaked()}.
	 * The breaking may not be further cancelled.
	 */
	 ...
	 * {@link #openFilter(FilterTask)} dependless
	 * on the type of invocation: plain or recursive.
	 *
	 * The breaking of the cyclehas no effect here,
